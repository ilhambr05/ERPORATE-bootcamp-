	...
	<div class="wrapper col-md-10 col-md-offset-1">
		...
		<div class="main main-raised">
			<div class="section section-basic">
					<h3>Absen Karyawan     <button class="btn btn-primary" style="margin-left: 20px;" data-toggle="modal" data-target="#modal_add">
	                      Tambah Baru
	                    </button></h3>
            ...
            <div class="modal-body">
                <form class="form-horizontal" role="form">
                  <div class="form-group">
                    ...
                    <div class="col-sm-8">
                        <input type="text" class="form-control" name="nama" placeholder="Nama karyawan harus cocok dengan database"/>
                    </div>
                  </div>
                  <div class="form-group">
                  ...
                  <div class="form-group">
                    <label  class="col-sm-2 control-label">Datang-Pulang</label>
                    <div class="col-sm-4">
                        <input type="text" class="form-control" name="jam_datang" placeholder="jam_datang 00:00"/>
                    </div>
                    <div class="col-sm-4">
                        <input type="text" class="form-control" name="jam_pulang" placeholder="jam_pulang 00:00"/>
                    </div>
                  </div>
                </form>    
            </div>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
        ...
        $('[name="tgl"').attr("value",today);
	});
	function muat(){
		$.ajax({
			url : "./crud/loadAbsen.php",
			type: "GET",
	...
	function buatTabel(json){
		isi = "<table class='table '>";
	      ...
	      //thead tr th
	      isi+="<thead><tr>";
	      isi += "<th>ID</th><th>Nama</th><th>Hari, Tanggal</th><th>jam_datang</th><th>jam_pulang</th><th>jam_kerja</th><th class='text-right'>Action</th>";
	      /*for(x in header){
	        isi += "<th>"+header[x]+"</th>";
	    }*/
	    isi+="</tr></thead>";
	      ...
	      for(x in json){
	      	...
	      	isi+="<td class='data_id'>"+json[x].id_kehadiran+"</td>";
	      	isi+="<td>"+json[x].nama_karyawan+"</td>";
	      	isi+="<td>"+json[x].hari_tgl+"</td>";
	      	isi+="<td>"+json[x].jam_datang+"</td>";
	      	isi+="<td>"+json[x].jam_pulang+"</td>";
	      	isi+="<td>"+json[x].jam_kerja+"</td>";
	    	...
	    	isi+='<td>'+data[x].id_kehadiran+'</td>';
	    	isi+="<td>"+json[x].nama_karyawan+"</td>";
	      	isi+="<td>"+json[x].hari_tgl+"</td>";
	      	isi+='<td><div class="form-group edit"><input type="text" class="form-control" value="'+data[x].jam_datang+'"><span class="material-input"></span></div></td>';
	    	isi+='<td><div class="form-group edit"><input type="text" class="form-control" value="'+data[x].jam_pulang+'"><span class="material-input"></span></div></td>';
	    	isi+="<td>"+json[x].jam_kerja+"</td>";
    ...
    function add_data(){
    	data = {
    		nama:$("[name='nama']").val(),
    		tgl:$("[name='tgl']").val(),
    		jam_datang:$("[name='jam_datang']").val(),
    		jam_pulang:$("[name='jam_pulang']").val(),
    	}
		//console.log(data);
		$.ajax({
			url : "./crud/insertAbsen.php",
			type: "POST",
			data: data,
			success: function(result)
			{
    ...
    function confirm_edit(index){
    	data = {
    		...
    		date:$('tr:eq('+(index*2)+')>td').eq(2).html(),
    		jam_datang:$('input:visible').eq(0).val(),
    		jam_pulang:$('input:visible').eq(1).val(),
    	}
    	console.log(data);
    	$.ajax({
			url : "./crud/editAbsen.php",
			type: "POST",
			data: data,
			success: function(result)
			{
    ...
    function cancel(x){
    	$('thead>tr').html("<th>ID</th><th>Nama</th><th>Jk</th><th>jam_datang</th><th>No HP</th><th>jam_kerja</th><th class='text-right'>Action</th>");
    	$('.data_edit').eq(x).hide();
    	$('.data_real').eq(x).show();
    	adaProses= false;
    }
